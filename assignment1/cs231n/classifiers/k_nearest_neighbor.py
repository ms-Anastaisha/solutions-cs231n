...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        dists = np.sqrt(np.repeat((X**2).sum(axis=1)[:,np.newaxis], self.X_train.shape[0], 1) 
          +(self.X_train**2).sum(axis=1)- 2 * np.dot(X, self.X_train.T))
