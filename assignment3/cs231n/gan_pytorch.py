...
def discriminator_loss(logits_real, logits_fake):
    ...
    true_labels_real = torch.ones(N).type(dtype)
    true_labels_fake = torch.zeros(N).type(dtype)
    real_loss = bce_loss(logits_real, true_labels_real)
    fake_loss = bce_loss(logits_fake, true_labels_fake)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = (0.5*(scores_real - 1).pow(2) + 0.5*scores_fake.pow(2)).mean()
